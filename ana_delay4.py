"""
bsub -n 64 -o ~/lr25_code/output/%J.log -q psanaq  mpirun python ana_delay4.py

bsub -n 64 -o ~/lr25_code/output/%J.log -q psnehhiprioq  mpirun python ana_delay4.py --run 177

day 5 version, good for 4th of dec 17, files 178+
...
run_default = 177  #if not specified by --run
...
energy1_mean = 515
energy2_mean = 725
energy1_min = energy1_mean-35
energy1_max = energy1_mean+35
energy2_min = energy2_mean-35
energy2_max = energy2_mean+35
...
for nfi,fi in enumerate(files):
	...
	for evt in range(evtnr):
		...
		His2d_w.fill(list(data['GasDetector'][evt,0:2].mean()*np.ones(714)),list(UXSpc[0]*np.ones(714)),\
			list(UXSpc[3]*np.ones(714)),list(UXSpc[2]*np.ones(714)), \
			list(UXSpc[5]*np.ones(714)),range(714),weights=values/data['GasDetector'][evt,0:2].mean())
...
if rank==0:
	filename = "/reg/neh/home5/sjarosch/lr25_code/npz/ana_delay4_run_%04d" %run
